...
def lunet_small(nClasses = nClasses, optimizer='Adam', input_width=256, input_height=256, nChannels=3, nTime = 2):
    ...
    conv_lstm1 = ConvLSTM2D(16, kernel_size=(3, 3), activation='sigmoid', padding='same', input_shape=(nTime, input_width, input_height, nChannels), data_format='channels_last', return_sequences=True, dropout=0.2 ,recurrent_dropout = 0.2)(inputs)
    conv_lstm1 = ConvLSTM2D(16, kernel_size=(3, 3), activation='sigmoid', padding='same', data_format='channels_last', return_sequences=True)(conv_lstm1)
    Norm1 = BatchNormalization()(conv_lstm1)
    pooling1 = MaxPooling3D(pool_size=(1, 2, 2))(Norm1)

    conv_lstm2 = ConvLSTM2D(32, kernel_size=(3, 3), activation='sigmoid', padding='same', data_format='channels_last', return_sequences=True, dropout=0.2 ,recurrent_dropout = 0.2)(pooling1)
    conv_lstm2 = ConvLSTM2D(32, kernel_size=(3, 3), activation='sigmoid', padding='same', data_format='channels_last', return_sequences=True)(conv_lstm2)
    Norm2 = BatchNormalization()(conv_lstm2)
    pooling2 = MaxPooling3D(pool_size=(1, 2, 2))(Norm2)

    conv_lstm3 = ConvLSTM2D(64, kernel_size=(3, 3), activation='sigmoid', padding='same', data_format='channels_last', return_sequences=True, dropout=0.2 ,recurrent_dropout = 0.2)(pooling2)
    conv_lstm3 = ConvLSTM2D(64, kernel_size=(3, 3), activation='sigmoid', padding='same', data_format='channels_last', return_sequences=True)(conv_lstm3)
    Norm3 = BatchNormalization()(conv_lstm3)

    up1 = concatenate([UpSampling3D(size=(1, 2, 2))(Norm3), Norm2], axis=4)
    conv_lstm4 = ConvLSTM2D(32, kernel_size=(3, 3), activation='sigmoid', padding='same', data_format='channels_last', return_sequences=True, dropout=0.2 ,recurrent_dropout = 0.2)(up1)
    conv_lstm4 = ConvLSTM2D(32, kernel_size=(3, 3), activation='sigmoid', padding='same', data_format='channels_last', return_sequences=True)(conv_lstm4)
    Norm4 = BatchNormalization()(conv_lstm4)

    up2 = concatenate([UpSampling3D(size=(1, 2, 2))(Norm4), conv_lstm1], axis=4)
    conv_lstm5 = ConvLSTM2D(16, kernel_size=(3, 3), activation='sigmoid', padding='same', data_format='channels_last', return_sequences=True, dropout=0.2 ,recurrent_dropout = 0.2)(up2)
    conv_lstm5 = ConvLSTM2D(16, kernel_size=(3, 3), activation='sigmoid', padding='same', data_format='channels_last', return_sequences=False)(conv_lstm5)
...
model.summary()
model.load_weights("checkpoint_30_acc_0.95.hdf5")

image_path1 = r"D:\change_detection_dataset\time1\images\1_76_228.png"
image_path2 = r"D:\change_detection_dataset\time2\images\1_76_228.png"
label_path = r"D:\change_detection_dataset\label\images\1_76_228.png"

image1 = np.array(Image.open(image_path1))
image2 = np.array(Image.open(image_path2))
label = np.array(Image.open(label_path)).astype("float32")/255

lstm_input = np.array([image1,image2]).astype("float32")/255
...
output = (y.squeeze()*255)
image_output = Image.fromarray(np.uint8(output))
image_output.show()
print(y.shape)
